...
try:
    with open(os.path.join(BASE_DIR, "data", "caprica_lore.md"), "r", encoding="utf-8") as f:
        CAPRICA_LORE = f.read()
except Exception:
    CAPRICA_LORE = ""
    ...
    async for message in channel.history(limit=50):
        ...
        if message.author.id == BOT_ID and message.embeds:
            ...
            if existing:
                ...
                # Check if we need to fix existing votes (like missing abstain)
                needs_vote_fix = False
                if not needs_votes and votes_yay is not None:
                    conn_fix = sqlite3.connect(DB_PATH, timeout=15)
                    c_fix = conn_fix.cursor()
                    c_fix.execute("SELECT votes_abstain FROM bills WHERE id = ?", (bill_id,))
                    existing_abs = c_fix.fetchone()
                    conn_fix.close()
                    if existing_abs and existing_abs[0] is None and votes_abstain is not None:
                        needs_vote_fix = True

                if title != existing_main_goal and existing_main_goal == "Pending analysis.":
                ...
                elif needs_votes or needs_vote_fix:
